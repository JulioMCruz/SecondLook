...
export default function EvidenceReport({check}: {check: CheckRecord}) {
  ...
  return <section className="space-y-6" aria-label={es ? "Expediente de evidencia" : "Evidence brief"}>
    <div className="rounded-2xl border border-[#b7cfc2] bg-white p-6 sm:p-8">
      ...
      <div className="mt-6 grid grid-cols-3 gap-2 sm:gap-3">{counts.map(({status,count}) => <div key={status} className="rounded-xl bg-[#f6f8f6] px-3 py-3 sm:px-4"><p className="text-2xl font-semibold tabular-nums">{count}</p><p className="mt-1 text-xs text-[var(--muted)]">{labels[status]}</p></div>)}</div>
    </div>
