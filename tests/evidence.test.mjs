...
test('fabricated citation id and fabricated excerpt cannot support a finding',()=>{for(const evidence of [[{sourceId:'FAKE',excerpt:source.snippet,relation:'supports'}],[{sourceId:'F1',excerpt:'Everyone uses this product already.',relation:'supports'}]]){const b=parseEvidence({...base,findings:[{...base.findings[0],evidence}]},[source],'en');assert.equal(b.findings[0].status,'insufficient');assert.equal(b.findings[0].evidence.length,0);}});
test('context alone cannot support a claim',()=>{const b=parseEvidence({...base,findings:[{...base.findings[0],evidence:[{...base.findings[0].evidence[0],relation:'context'}]}]},[source],'en');assert.equal(b.verdict,'Unknown');});
test('malformed findings fail rather than create a success report',()=>{assert.throws(()=>parseEvidence({findings:[null]},[source],'en'));});
test('source selection limits repeated domains and total context',()=>{const s=Array.from({length:30},(_,i)=>({...source,url:`https://${i<10?'vendor':i}.example.com/${i}`}));const r=curateSources(s);assert.ok(r.length<=12);assert.equal(r.filter(x=>x.url.includes('vendor.')).length,2);});
const record = {status:'unlocked',userId:'u',payload:{firstLook:{sources:[]},brief:{summary:'paid'},followUp:{sources:[]},metrics:{},entitlementActive:true}};
